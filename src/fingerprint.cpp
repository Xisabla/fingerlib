...
{
    std::map<std::string, int> counter = getCounter(bstr);
    float entropy = 0;
    for (auto& item : counter)
    {
        float p = item.second / (float)bstr.size();
        entropy += p * log2(p);
    }
    return -entropy;
}
...
{
    return std::to_string(std::round(entropy(bstr) * 10) / 10);
}

std::string getLog10Length(const std::vector<std::string>& bstr)
{
    return std::to_string(std::round(log10(bstr.size()) * 10) / 10);
}
...
std::string getHeaderValue(const std::string& header, const std::string& headerName, const std::map<std::string, std::string>& headerValues)
{
    ...
    if (val.find(',') != std::string::npos)
    {
        ...
        if (val.find(";q=") != std::string::npos)
        {
            ...
            return headerCoded + std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(val.encode()), "x")
        }
    ...
    else
    {
        ...
        else
        {
            ...
            std::cout << "Unknown header value - " << header << std::endl;
            #endif
            k = std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(val.encode()), "x")
        }
        ret.push_back(k);
    }
        ...
        for (auto& itv : vals)
        {
            if (itv.find(";") != std::string::npos)
            {
                if (itv.find("boundary=") != std::string::npos)
                {
                    ...
                    return hdr_coded + std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(val_bnd.encode()), "x")
                }
                else
                {
                    ret.push_back(std::to_string(29)); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(itv.encode()), "x")
                }
            }
            else
            {
                std::string k = std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(itv.encode()), "x")
    ...
    else
    {
        if (val.find(";") != std::string::npos)
        {
            if (val.find("boundary=") == std::string::npos)
                return hdr_coded + std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(val.encode()), "x")
        ...
        {
            std::string k = std::to_string(29); // TODO : replace with corresponding FNV1a_32 hash -> format(fnv1a_32(val.encode()), "x")
...
std::string getAcceptLanguageValue(const std::string& header)
{
    ...
    int fnv1a_32 = 29; // TODO : replace with corresponding FNV1a_32 hash
    std::string ret = name + ":" + std::to_string(fnv1a_32); //format(fnv1a_32(val.encode()), "x")
    return ret;
}
