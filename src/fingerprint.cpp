...
std::string getMethodVersion(const std::vector<std::string>& requestSplit) {
    ...
    if (requestSplit[0].find(" HTTP/") == std::string::npos) {
        ...
        // take first seven characters of the first line of request to look for method (methods have
        // up to 7 chars)
...
std::string getHeaderValue(const std::string& header,
    ...
    if (val.find(',') != std::string::npos) {
        ...
        if (val.find(";q=") != std::string::npos) {
            ...
            return headerCoded + std::to_string(29); // TODO : replace with corresponding FNV1a_32
                                                     // hash -> format(fnv1a_32(val.encode()), "x")
        }
